...
subsets:
  - addresses:
    ...
    ports:
      - port: 443
EOF
        assert_equals 403 "$(KUBECONFIG="$KUBECONFIG2" retry 10 5 "service is not yet routable" curl_pod -m 1 -s -o /dev/null -k -w "%{http_code}" https://mirrored-kubernetes)" "should be able to make HTTP request from cluster 2 to service in cluster 1"
}
